...
* `learner`: Currently, `SuperLearner` and `xgboost` are available. Each has its own control parameters, which are detailed below. 
...
* `xgb_control`: This is a named list of arguments that are use to fit the `xgboost` algorithm. `tune` is a logical indicating whether to use cross-validation to select from a set of possible tuning parameters, or to fit `xgboost` using a single pre-specified set of parameters. `tuning_params` is a list of tuning parameters passed to `xgboost`. See the `xgboost` package documentation for more information.  
...
Charles J. Wolock, Peter B. Gilbert, Noah Simon and Marco Carone. "A framework for leveraging machine learning tools to estimate personalized survival curves." [arXiv: 2211.03031.](https://arxiv.org/abs/2211.03031) 
